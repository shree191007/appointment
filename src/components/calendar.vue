        ...
        <v-card>
          <v-container>
            <v-form @submit.prevent="addEvent">
              ...
              <v-text-field v-model="start_time" type="time" label="start time (required)"></v-text-field>
  ...
  methods: {
    ...
    async addEvent () {
      if (this.name && this.start && this.end) {
        ...
        axios.post("http://localhost:5000/appointments",{name: this.name,
    ...
    async deleteEvent (ev) {
      axios.delete(`"http://localhost:5000/appointments/${ev.id}"`) 
